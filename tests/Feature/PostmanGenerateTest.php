...
namespace Akkurateio\LaravelPostman\Tests\Feature;

class PostmanGenerateTest extends TestCase
{
    protected function setUp(): void
    {
        parent::setUp();

        config()->set('postman.filename', 'test.json');
    ...
    {
        config()->set('postman.enable_formdata', $formDataEnabled);

        $this->artisan('postman:generate')->assertExitCode(0);

        $collection = json_decode(Storage::get('postman/'.config('postman.filename')), true);
    ...
    public function test_bearer_export_works(bool $formDataEnabled)
    {
        ...
        $this->artisan('postman:generate --bearer=1234567890')->assertExitCode(0);
    ...
    public function test_basic_export_works(bool $formDataEnabled)
    {
        ...
        $this->artisan('postman:generate --basic=username:password1234')->assertExitCode(0);
    ...
    {
        config([
            'postman.structured' => true,
            'postman.enable_formdata' => $formDataEnabled,
        ]);
    ...
    {
        config([
            'postman.enable_formdata' => true,
            'postman.print_rules' => true,
        ...
        $collection = collect(json_decode(Storage::get('postman/'.config('postman.filename')), true)['item']);
    ...
    public function test_rules_printing_export_to_human_readable_works()
    {
        config([
            'postman.enable_formdata' => true,
            'postman.print_rules' => true,
            'postman.rules_to_human_readable' => true,
        ]);
